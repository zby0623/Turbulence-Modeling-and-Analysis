...
scale_factor = 1.0;
w0 = 0.175E-2; % beam waist radius
delta = 100E-6 * scale_factor;
N=2047;
wmax=delta*N;
...
for i=0:1:2
    for j=1:length(cn2)
        ...
        for i_realization_index = 1 : realizationNumber % A weak ambient turbulence (Cn2<1E-13) will result in singularities in the calculation of Cn2r0, 
            % though the simulation results will not be affected, it is better to directly calculation with one phase screen
            if j==1
                r0=(0.423*k^2*cn2*0.15)^(-3/5);
