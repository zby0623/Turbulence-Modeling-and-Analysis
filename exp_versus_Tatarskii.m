...
cn2=0.3928/0.3267*2E-13*(5:5:50).^2.0022;
cn2=[1.7970E-14 cn2];
for i=1:3
...
cn2=[1.7970E-14 0.3928/0.3267*2E-13*[15 30 40 50].^2.0022];
for i=1:3
    errorbar(cn2,Ecc_exp(i,:),Ecc_exp_std(i,:),shape_exp(i),'MarkerSize',ms,'MarkerFaceColor','auto','LineWidth',1.5);
    hold on;
end 
hold off;
legend('m=0 Tatarskii','m=1 Tatarskii','m=2 Tatarskii','m=0 exp','m=1 exp','m=2 exp','NumColumns',2,'Location','North');
xlabel('C_{n}^{2} (m^{-2/3})');
ylabel('Eccentricity');
axis([0 0.61E-9 0 1]);
set(gca,'FontSize',fs);
saveas(gca,'ecc_tar.pdf','pdf');
figure(2)
load("simulation_result_all_1009_Tatarskii.mat");
cn2=0.3928/0.3267*2E-13*(5:5:50).^2.0022;
cn2=[1.7970E-14 cn2];
...
hold off;
legend('m=0 Tatarskii','m=1 Tatarskii','m=2 Tatarskii','m=0 exp','m=1 exp','m=2 exp','NumColumns',2,'Location','South');
xlabel('C_{n}^{2} (m^{-2/3})');
ylabel('Ellipticity');
axis([0 0.61E-9 0.6 1]);
set(gca,'FontSize',fs);
saveas(gca,'ell_tar.pdf','pdf');